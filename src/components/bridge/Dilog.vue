<template>
	<div class="brigedilog">
		<el-dialog title="梁片出库" :visible.sync="$store.state.bridgechange" width="50%">
			<ul class="top">
				<li>梁片ID：{{bridgeinfo.girderId}}</li>
				<li>审批状态：{{bridgeinfo.audit | audit}}</li>
				<li>审批意见：{{bridgeinfo.comments}}</li>
				<li>出库状态：{{bridgeinfo.outboundStatus | outboundStatus}}</li>
				<li>监理单位：{{bridgeinfo.supervisorDeptName}}</li>
				<li>监理工程师：{{bridgeinfo.supervisorName}}</li>
			</ul>
			<div class="bow">
...
<script>
	import {outStock} from "@/utils/synthesize"
	export default {
		inject:["reload"],
		props:["bridgeinfo"],
		filters:{
		...
		methods: {
			...
			subbridge() {
				let obj={
					...
					outboundDate:this.sizeForm.outboundDate,
					outboundStatus:140001,
					outboundAddress:this.sizeForm.outboundAddress
				}
				if(obj.outboundDate == null){
					this.$message.error('请输入出库时间')
					return false
				}else if(obj.outboundAddress == null){
					this.$message.error('请输入出库地址')
					return false
				}else{
					outStock(obj).then(res=>{
						if(res.code == 200){
							this.$message.success('出库成功')
							this.$store.state.bridgechange=false
							this.reload()
						}else{
