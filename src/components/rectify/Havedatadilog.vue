<template>
	<el-dialog title="发起检查修改" :visible.sync="$store.state.havedatadilog" width="80%" :before-close="close">
		<!-- <div v-if="ruleForm.status==10004"> -->
				...
				<div class="addbox" v-for="(item, index) in ruleForm.rectifyProblemUpdates" :key="index">
					...
					<el-form-item label="附件">
						...
						:action="baseurl+'v1/communal/files/scopes/rectify/attachment/actions/upload'"
							:on-success="handlePdfAdd.bind(this,index)" :on-preview="previewPdf" :file-list="item.fileListPdf"
				...
				<el-row>
					...
					<el-col :span="12">
					 <el-form-item label="完成时间" prop="finishTime">
						...
						<el-form-item label="部门" prop="processorDeptId" v-if="bumenbox">
			...
			<span slot="footer" class="dialog-footer" style="text-align: center;display: block;">
				...
				<el-button type="success" @click="savefrom('ruleForm')">保存修改</el-button>
				<el-button type="primary" @click="submitForm('ruleForm')" v-if="ruleForm.status == 10004">发起整改</el-button>
			</span>
		<!-- </div> -->
	</el-dialog>
</template>
...
<script>
	import {biaoduan,getUserList,regiletoformal,savechange,reformeid,cahngzcon} from "@/utils/rectify"
	export default {
		inject:["reload"],
		props:["havedatainfo"],
		filters:{
		...
		methods: {
			...
			// 修改子内容
			updatazicon(val){
				  let arr1 = this.ruleForm.rectifyProblemUpdates.filter((element, index, self) => {
				            return element.problemId == val
				        })
				cahngzcon({
			...
			handleRemove(index, file, fileList) {
				...
				this.ruleForm.rectifyProblemUpdates[index].fileListImg = fileList.map(item => {
					   return item
				})
			},
			//删除pdf
			handlePdfRemove(index, file, fileList) {
				...
				this.ruleForm.rectifyProblemUpdates[index].fileListPdf = fileList.map(item => {
					   return item
				})
			},
			// 放大图片
